$(document).ready(function () {
  ...
  function setupImpoundForm(data) {
    ...
    $('#reason').attr('placeholder', `Rédigez une description d'au moins ${rules.minReasonLength} caractères`);
  }
  ...
  function validateImpoundForm() {
    ...
    if (fee.isNaN || String(fee).length < 1 || parseInt(fee) < rules.minFee || parseInt(fee) > rules.maxFee) {
      errors.append(
        `<small>&#9679; L'amende ne peut être inférieure à ${rules.minFee} ni supérieure à ${rules.maxFee}.</small>`
      );
      success = false;
    }
    ...
    if (reason.length < rules.minReasonLength || reason.length > 10000) {
      errors.append(
        `<small>&#9679; Vous devez indiquer une raison valide d'au moins ${rules.minReasonLength} caractères.</small>`
      );
      success = false;
    }
    ...
    for (var i = 0; i < data.vehicles.length; i++) {
      ...
      if (data.money < data.vehicles[i].fee) {
        button = `<td>
					...
					<button class="btn pay success" disabled>PAS D'ARGENT</button>
				</td></tr>`;
      } else {
        button = `<td>
					<button class="btn info mr" id="info${i}">INFO</button>
					<button class="btn pay success" id="${i}">PAYER</button>
				</td></tr>`;
      }
