$(document).ready(function () {
  ...
  function setupImpoundForm(data) {
    ...
    $('#reason').attr(
      'placeholder',
      `Escribe una descripción de al menos ${rules.minReasonLength} caracteres`
    );
  }
  ...
  function validateImpoundForm() {
    ...
    if (fee.isNaN || String(fee).length < 1 || parseInt(fee) < rules.minFee || parseInt(fee) > rules.maxFee) {
      errors.append(
        `<small>&#9679; La multa no puede ser menos de ${rules.minFee} ni mas de ${rules.maxFee}</small>`
      );
      success = false;
    }

    if (reason.length < rules.minReasonLength || reason.length > 255) {
      errors.append(
        `<small>&#9679; Debes colocar una razón válida de al menos ${rules.minReasonLength} caractéres y máximo 255.</small>`
      );
      success = false;
    }
    ...
    for (var i = 0; i < data.vehicles.length; i++) {
      ...
      if (data.money < data.vehicles[i].fee) {
        button = `<td>
					...
					<button class="btn pay success" disabled>SIN DINERO</button>
				</td></tr>`;
      } else {
        button = `<td>
					<button class="btn info mr" id="info${i}">INFO</button>
					<button class="btn pay success" id="${i}">PAGAR</button>
				</td></tr>`;
      }
